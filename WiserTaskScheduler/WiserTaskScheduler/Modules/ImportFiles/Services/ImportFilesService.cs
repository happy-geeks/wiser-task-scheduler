...
public class ImportFilesService(ILogService logService, ILogger<ImportFilesService> logger) : IImportFilesService, IActionsService, IScopedService
{
    /// <inheritdoc />
    ...
    private async Task<JObject> ImportSingleFileAsync(ImportFileModel importFile, string filePath, string configurationServiceName)
    {
        try
        {
            ...
            JObject importResult = importFile.FileType switch
            {
                ...
                _ => throw new NotImplementedException($"File type '{importFile.FileType}' is not yet implemented to be imported.")
            };
