...
/// </summary>
public class QueriesService(ILogService logService, ILogger<QueriesService> logger, IServiceProvider serviceProvider, IOptions<GclSettings> gclSettings) : IQueriesService, IActionsService, IScopedService
{
    ...
    private async Task<JObject> Execute(QueryModel query, IDatabaseConnection databaseConnection, JObject resultSets, string configurationServiceName)
    {
        ...
        for (var i = 0; i < usingResultSet.Count; i++)
        {
            ...
            {
                var secondLayerKey = keyWithSecondLayer[..keyWithSecondLayer.IndexOf("[j]", StringComparison.Ordinal)];
