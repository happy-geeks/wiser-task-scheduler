...
public class WiserImportsService(
    IDatabaseConnection databaseConnection, 
    IWiserItemsService  wiserItemsService,
    ITaskAlertsService   taskAlertsService,
    ILogService logService, 
    ILogger<WiserImportsService> logger) 
    : IWiserImportsService, IActionsService, IScopedService
{
    ...
    public async Task<JObject> Execute(ActionModel action, JObject resultSets, string configurationServiceName)
    {
        ...
        await logService.LogInformation(
            ...
            wiserImport.LogSettings, 
            $"Starting the import for '{databaseName}'.", 
        ...
        var importDataTable = await GetImportsToProcessAsync();
        if (importDataTable.Rows.Count == 0)
        {
            await logService.LogInformation(
                ...
                LogScopes.RunStartAndStop, 
                wiserImport.LogSettings, 
                $"Finished the import for '{databaseName}' due to no imports to process.", 
                configurationServiceName, 
                wiserImport.TimeId, 
                wiserImport.Order);
        ...
        var importRows = importDataTable
            .Rows
            .Cast<DataRow>()
            .Select(dataRow => new ImportRowModel(dataRow))
            .ToList();

        await HandleInvalidImportRowsAsync(importRows, wiserImport, databaseName, configurationServiceName);
        
        var validImportRowModels = importRows
            .Where(row => string.IsNullOrEmpty(row.RawData))
            .ToList();
        ...
        foreach (var importRowModel in validImportRowModels)
        {
            stopwatch.Restart();
            ...
            var usernameForLogs = $"{importRowModel.Username} (Import)";
            var startDate = DateTime.Now;
            
            await SaveStartTimeAsync(importRowModel.Id, startDate);

            var importData = JsonConvert.DeserializeObject<List<ImportDataModel>>(importRowModel.RawData, serializerSettings);
            foreach (var import in importData)
            {
                try
                {
                    import.Item = await wiserItemsService.SaveAsync(import.Item, import.Item.ParentItemId, username: usernameForLogs, userId: importRowModel.UserId);
                }
                catch (Exception e)
                {
                    await logService.LogError(
                        ...
                        LogScopes.RunBody, 
                        wiserImport.LogSettings, 
                        $"Failed to import an item due to exception:\n{e}\n\nItem details:\n{JsonConvert.SerializeObject(import.Item, serializerSettings)}", 
                        configurationServiceName, 
                        wiserImport.TimeId, 
                        wiserImport.Order);
                ...
                await ImportLinksAsync(wiserImport, import, usernameForLogs, errors, destinationLinksToKeep, itemParentIdsToKeep, sourceLinksToKeep, importRowModel, configurationServiceName);
                await ImportFilesAsync(wiserImport, import, usernameForLogs, errors, importRowModel, configurationServiceName);
            }

            // If there have been no errors the import was successful. We can safely clean up any links that need to be deleted.
            if (errors.Count == 0)
            {
                var entityType = importData.First().Item.EntityType;
                
                var destinationRemovalErrors = await RemoveDestinationLinks(
                    wiserImport,
                    destinationLinksToKeep, 
                    usernameForLogs, 
                    importRowModel, 
                    configurationServiceName, 
                    entityType);
                
                var sourceRemovalErrors = await RemoveSourceLinks(
                    wiserImport,
                    destinationLinksToKeep, 
                    usernameForLogs, 
                    importRowModel, 
                ...
                var parentRemovalErrors = await RemoveParentLinks(
                    wiserImport,
                    destinationLinksToKeep, 
                    usernameForLogs, 
                    importRowModel, 
                ...
                errors.AddRange([..destinationRemovalErrors, ..sourceRemovalErrors, ..parentRemovalErrors]);
                successfulImports++;
            }
            else
            {
            ...
            await SaveEndTimeAsync(importRowModel.Id, endDate, errors);

            var replaceData = new Dictionary<string, object>
            {
                {"name", importRowModel.ImportName},
                {"date", DateTime.Now},
            ...
            var wiserItemModel = wiserImport.TemplateId > 0
                ? await wiserItemsService.GetItemDetailsAsync(wiserImport.TemplateId, userId: importRowModel.UserId)
                : null;

            var subject = wiserItemModel?.GetDetailValue("subject");
            if (string.IsNullOrWhiteSpace(subject))
                subject = DefaultSubject;

            var content = wiserItemModel?.GetDetailValue("template");
            if (string.IsNullOrWhiteSpace(content))
                content = DefaultContent;

            await taskAlertsService.NotifyUserByEmailAsync(
                importRowModel.UserId, 
                importRowModel.Username, 
                wiserImport, 
                configurationServiceName, 
                subject, 
                content, 
                replaceData, 
                wiserItemModel?.GetDetailValue("sender_email"), 
                wiserItemModel?.GetDetailValue("sender_name"));
            
            await taskAlertsService.SendMessageToUserAsync(
                importRowModel.UserId, 
                importRowModel.Username, 
                subject, 
                wiserImport, 
    ...
    private static string GetDatabaseNameForLogging(string connectionStringToUse)
    {
        var databaseConnectionStrings = connectionStringToUse
            .Split(';')
            .Where(part => part.StartsWith("database"))
            .ToList();
        
        return databaseConnectionStrings.Count > 1 
            ? databaseConnectionStrings[0].Split('=')[1] 
            : "Unknown";
    }
    ...
    private async Task<DataTable> GetImportsToProcessAsync()
    {
    ...
    /// <param name="importRow">The information of the import itself.</param>
    /// <param name="configurationServiceName">The name of the configuration the import is executed within.</param>
    private async Task ImportFilesAsync(WiserImportModel wiserImport, ImportDataModel import, string usernameForLogs, List<string> errors, ImportRowModel importRow, string configurationServiceName)
    {
    ...
    private async Task<List<string>> RemoveDestinationLinks(WiserImportModel wiserImport, Dictionary<int, Dictionary<ulong, List<ulong>>> destinationLinksToKeep, string usernameForLogs, ImportRowModel importRow, string configurationServiceName, string entityType)
    {
        var errors = new List<string>();
        
        foreach (var (linkType, dictionary) in destinationLinksToKeep)
        {
            foreach (var (destinationItemId, value) in dictionary)
            {
                ...
                try
                {
                    ...
                    var destinationLinksToDelete = await databaseConnection.GetAsync($"SELECT `id`, item_id AS sourceId, destination_item_id AS destinationId FROM {linkPrefix}{WiserTableNames.WiserItemLink} WHERE type = ?type AND destination_item_id = ?destinationItemId AND item_id NOT IN ({String.Join(",", value)})");
    ...
    private async Task<List<string>> RemoveParentLinks(WiserImportModel wiserImport, Dictionary<int, Dictionary<ulong, List<ulong>>> itemParentIdsToKeep, string usernameForLogs, ImportRowModel importRow, string configurationServiceName, string entityType)
    {
        var errors = new List<string>();
        
        foreach (var (linkType, value) in itemParentIdsToKeep)
        {
            foreach (var (parentItemId, ulongs) in value)
            {
                var linkSettings = await wiserItemsService.GetLinkTypeSettingsAsync(linkType, entityType);
                ...
                try
                {
                    ...
                    foreach (var linkedItem in linkedItems.Where(linkedItem => !ulongs.Contains(linkedItem.Id)))
                    {
                    ...
                    if (linkIds.Count != 0)
                    {
                        await wiserItemsService.RemoveParentLinkOfItemsAsync(
                            linkIds.ToList(), 
                            linkSettings.SourceEntityType, 
                            sourceIds.ToList(), 
                            linkSettings.DestinationEntityType, 
                ...
                catch (Exception e)
                {
                    await logService.LogError(
                        ...
                        LogScopes.RunBody, 
                        wiserImport.LogSettings, 
                        $"Error while trying to delete item links (parent) of type '{linkType}' due to exception:\n{e}", 
                        configurationServiceName, 
                        wiserImport.TimeId, 
                        wiserImport.Order);
    ...
    private async Task<List<string>> RemoveSourceLinks(WiserImportModel wiserImport, Dictionary<int, Dictionary<ulong, List<ulong>>> sourceLinksToKeep, string usernameForLogs, ImportRowModel importRow, string configurationServiceName, string entityType)
    {
        var errors = new List<string>();
        
        foreach (var (linkType, value) in sourceLinksToKeep)
        {
            foreach (var (sourceItemId, ulongs) in value)
            {
                try
                {
                    ...
                    var sourceLinksToDelete = await databaseConnection.GetAsync(
                        $"SELECT `id`, item_id AS sourceId, destination_item_id AS destinationId " +
                                $"FROM {linkPrefix}{WiserTableNames.WiserItemLink} " +
                                $"WHERE type = ?type " +
                                $"AND item_id = ?sourceItemId " +
                                $"AND destination_item_id NOT IN ({string.Join(",", ulongs)})");

                    if (sourceLinksToDelete.Rows.Count <= 0)
                        continue;

                    var dataRows = sourceLinksToDelete
                        .Rows
                        .Cast<DataRow>()
                        .ToArray();
                    
                    var linkIds = dataRows
                        .Select(r => Convert.ToUInt64(r["id"]))
                        .Distinct()
                        .ToList();
                    
                    var destinationIds = dataRows
                        .Select(r => Convert.ToUInt64(r["destinationId"]))
                        .Distinct()
                        .ToList();
                    
                    var sourceIds = dataRows
                        .Select(r => Convert.ToUInt64(r["sourceId"]))
                        .Distinct()
                        .ToList();

                    await wiserItemsService.RemoveItemLinksByIdAsync(
                        linkIds, 
                        linkSettings.SourceEntityType, 
                        sourceIds, 
                        linkSettings.DestinationEntityType, 
                ...
                catch (Exception e)
                {
                    await logService.LogError(
                        ...
                        LogScopes.RunBody, 
                        wiserImport.LogSettings, 
                        $"Error while trying to delete item links (source) of type '{linkType}' due to exception:\n{e}", 
                        configurationServiceName, 
                        wiserImport.TimeId, 
                        wiserImport.Order);
    ...
    /// <summary>
    /// Logs all invalid import rows.
    /// A row is invalid when there is no data in the data column.
    /// </summary>
    /// <param name="importRows"></param>
    /// <param name="wiserImport"></param>
    /// <param name="databaseName"></param>
    /// <param name="configurationServiceName"></param>
    private async Task HandleInvalidImportRowsAsync(IEnumerable<ImportRowModel> importRows, WiserImportModel wiserImport, string databaseName, string configurationServiceName)
    {
        var invalidImportRows = importRows
            .Where(row => string.IsNullOrWhiteSpace(row.RawData));

        foreach (var invalidImportRow in invalidImportRows)
        {
            await logService.LogError(
                logger, 
                LogScopes.RunBody, 
                wiserImport.LogSettings, 
                $"Import for '{databaseName}' with ID '{invalidImportRow.Id}' failed because there is no data to import.", 
                configurationServiceName, 
                wiserImport.TimeId, 
                wiserImport.Order);
        }
    ...
    /// <summary>
    /// Sets the start date of the import in the database
    /// </summary>
    /// <param name="importId"></param>
    /// <param name="startDate"></param>
    private async Task SaveStartTimeAsync(int importId, DateTime startDate)
    {
        databaseConnection.AddParameter("id", importId);
    ...
    /// <summary>
    /// Sets the end date of the import in the database
    /// </summary>
    /// <param name="importId"></param>
    /// <param name="endDate"></param>
    /// <param name="errors"></param>
    private async Task SaveEndTimeAsync(int importId, DateTime endDate, List<string> errors)
    {
        databaseConnection.AddParameter("id", importId);
        databaseConnection.AddParameter("finishedOn", endDate);
        databaseConnection.AddParameter("success", errors.Count == 0);
