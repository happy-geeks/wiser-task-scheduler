...
public class ServerMonitorsService(IServiceProvider serviceProvider, ILogService logService, ILogger<ServerMonitorsService> logger) : IServerMonitorsService, IActionsService, IScopedService
{
    ...
    public async Task<JObject> Execute(ActionModel action, JObject resultSets, string configurationServiceName)
    {
        ...
        switch (monitorItem.ServerMonitorType)
        {
            ...
            default:
                throw new ArgumentOutOfRangeException(nameof(monitorItem.ServerMonitorType), monitorItem.ServerMonitorType.ToString(), null);
        }
    }
    ...
    private async Task<JObject> GetCpuUsageAsync(ServerMonitorModel monitorItem, int threshold, string configurationServiceName, ICommunicationsService gclCommunicationsService)
    {
        ...
        return monitorItem.CpuUsageDetectionType switch
        {
            CpuUsageDetectionTypes.ArrayCount => await GetCpuUsageArrayCountAsync(monitorItem, threshold, configurationServiceName, gclCommunicationsService),
            CpuUsageDetectionTypes.Counter => await GetCpuUsageCounterAsync(monitorItem, threshold, configurationServiceName, gclCommunicationsService),
            _ => throw new ArgumentOutOfRangeException(nameof(monitorItem.CpuUsageDetectionType), monitorItem.CpuUsageDetectionType.ToString(), null)
        };
    }
