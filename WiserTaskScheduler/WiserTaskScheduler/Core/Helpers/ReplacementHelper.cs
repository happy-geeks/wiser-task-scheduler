...
{
    public static List<int> EmptyRows { get; } = [0, 0];
    ...
    public static Tuple<string, List<ParameterKeyModel>, List<KeyValuePair<string, string>>> PrepareText(string originalString, JObject usingResultSet, string remainingKey, HashSettingsModel hashSettings, bool insertValues = true, bool htmlEncode = false)
    {
        ...
        {
            var startIndex = result.IndexOf("[{", StringComparison.Ordinal) + 2;
            ...
            if (key.Contains("[]"))
            {
                ...
                {
                    values.Add(GetValue(key[(lastKeyIndex + 1)..], [i], (JObject) usingResultSetArray[i], htmlEncode));
                }
    ...
    private static string GetKeyToArray(string remainingKey, string key)
    {
        ...
        if (lastKeyIndex > 0)
        {
            keyToArray.Append(key[..lastKeyIndex]);
        }
    ...
    {
        if (String.IsNullOrWhiteSpace(originalString) || parameterKeys.Count == 0)
        {
